...
CREATE OR REPLACE FUNCTION convert_geometry()
  ...
  INSERT INTO data.items(id, type, geometry, properties, assets, collection_id)
  VALUES(new.id, new.type, converted_geometry, new.properties, new.assets, new.collection_id);
  RETURN NEW;
  END;
  $BODY$
  LANGUAGE plpgsql;
